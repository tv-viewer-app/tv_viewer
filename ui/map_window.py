...
class MapWindow:
    ...
    def _dismiss_popup(self):
        """Dismiss any existing country popup before opening a new one."""
        try:
            if hasattr(self, '_popup_ref') and self._popup_ref and self._popup_ref.winfo_exists():
                self._popup_ref.destroy()
        except (tk.TclError, AttributeError):
            pass
        self._popup_ref = None

    def _on_marker_click(self, country: str, channels: List[dict]):
        # Bug #111: Cancel/dismiss any existing popup before showing a new one
        self._dismiss_popup()
