...
-- Allow service_role to delete channels (for populate script cleanup)
-- NOTE: anon DELETE intentionally NOT granted — use service_role key.
DO $$ BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies WHERE tablename = 'channels' AND policyname = 'channels_delete'
    ) THEN
        CREATE POLICY "channels_delete" ON channels FOR DELETE TO service_role USING (true);
    END IF;
END $$;
...
-- ║  Restricted to service_role — clients use direct INSERT/UPDATE instead.  ║
-- ╚═══════════════════════════════════════════════════════════════════════════╝
...
-- ║  Restricted to service_role — admin scripts only.                      ║
-- ╚═══════════════════════════════════════════════════════════════════════════╝
