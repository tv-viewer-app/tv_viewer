...
# Service-role key required for destructive operations (--clean).
# Set via environment variable — never commit this key to source.
    ...
    for i in range(0, len(payload), batch_size):
        ...
        try:
            async with session.post(
                url, json=batch, headers=headers(upsert=True),
...
async def main():
    ...
    if '--clean' in sys.argv:
        if not SUPABASE_SERVICE_KEY:
            print("\n[ERROR] --clean requires SUPABASE_SERVICE_ROLE_KEY env var.")
        ...
        async with aiohttp.ClientSession() as session:
            # Delete all rows using service_role key (anon DELETE is blocked)
            url = f'{SUPABASE_URL}/rest/v1/channels'
            service_hdrs = {
