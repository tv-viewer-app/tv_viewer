/// Static channel description data for the info feature.
/// Maps channel name patterns (lowercase) to 1-sentence descriptions.
///
/// To add a new description, add an entry to the [_descriptions] map.
/// Keys should be lowercase and match the most recognizable part of the channel name.
class ChannelDescriptions {
  ChannelDescriptions._(); // Prevent instantiation
  ...
  /// Get description for a channel by name (fuzzy match).
  ///
  ...
  static String? getDescription(String channelName) {
    final lower = channelName.toLowerCase().trim();

    // 1. Exact match (O(1) hash lookup)
    if (_descriptions.containsKey(lower)) return _descriptions[lower];

    // 2. Partial / contains match with word-boundary safety
    final words = lower.split(RegExp(r'[\s\-_/|,]+'));

    for (final entry in _descriptions.entries) {
      final key = entry.key;

      if (key.length < 4) {
        // Short keys must appear as a whole word to avoid false positives
        if (words.contains(key)) return entry.value;
      } else {
