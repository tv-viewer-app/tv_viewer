...
The service is **opt-in only** — it is enabled when ``SUPABASE_URL`` and
``SUPABASE_ANON_KEY`` are set **and** ``config.TELEMETRY_ENABLED`` is ``True``
(see Issues #65, #79, #114).  By default telemetry is off until the user
explicitly consents via the first-run dialog or Settings → Privacy.
...
try:
    ...
    _TELEMETRY_ON = getattr(_supabase_cfg, "TELEMETRY_ENABLED", False)
except Exception:
    _DEFAULT_URL = ""
    _DEFAULT_KEY = ""
    _TELEMETRY_ON = False
...
# Feature flag — enabled only when credentials are set AND user has opted in.
# Respects TELEMETRY_ENABLED from config (Issues #65, #79, #114).
ENABLED: bool = bool(SUPABASE_URL and SUPABASE_ANON_KEY and _TELEMETRY_ON)
...
def _load_or_create_device_id() -> str:
    ...
    try:
        ...
        fd = os.open(str(_DEVICE_ID_PATH), os.O_WRONLY | os.O_CREAT | os.O_TRUNC, 0o600)
        try:
