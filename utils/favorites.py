...
class FavoritesManager:
    ...
    def _save(self):
        """Save favorites to disk with restricted permissions (#106)."""
        try:
            data = {
                'favorites': list(self._favorites),
                'recent': self._recent[:MAX_RECENT]
            }
            # Atomic write: write to temp file then replace (prevents corruption)
            import tempfile
            ...
            try:
                ...
                # Restrict file permissions — owner read/write only (Issue #106)
                if sys.platform != 'win32':
